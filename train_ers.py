...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp', default='ers')
    ...
    log_name = f"{args.exp}/{args.data}/{file_format}.log"
    ...
    model.eval()
    for idx, (batch, label) in tqdm(enumerate(KG_Loader), total=len(KG_Loader), desc="Prefill Progress"):
        with torch.no_grad():
    ...
    for epoch in range(last_epoch + 1, args.num_epoch + 1):
        ...
        for batch, label in KG_Loader:
            ent_embs, rel_embs, loss_c = model()
            
            loss = 0
            loss_b = model.contrastive_loss_relation(rel_embs)
            loss += loss_b
            scores = model.score(ent_embs, rel_embs, batch.cuda())
            loss_f = loss_fn(scores, label.cuda())
            loss += loss_f
            loss += loss_c * 0.001
            tloss_b += loss_c.item()
        ...
        if (epoch) % args.valid_epoch == 0:
            ...
            if best_mrr < mrr:
                ...
                torch.save(
                    ...
                    f"./ckpt/{args.exp}/{args.data}/{log_name}_best.ckpt"
                )
            ...
            if (epoch) % 500 == 0:
                torch.save(
                    ...
                    f"./ckpt/{args.exp}/{args.data}/{log_name}_{file_format}_{epoch}.ckpt"
                )
